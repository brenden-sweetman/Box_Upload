...
#   however, generally the script will upload to the designated file to share directly with the teaching center
#   Please see the technical documentation for information on how the script works
...
### Main function ###
def main():
    global client, oauth, verbose, conf_path, folderID
    ...
    parser.add_argument("-i","--ID",help="Provide alternative box file ID for upload",type=int)
    args=parser.parse_args(sys.argv[1:])
    # Pull values from command line args
...
def upload(filePath):
    global client, verbose, folderID
    # try to upload file
    print(folderID)
    try:
        print ("Starting Upload to box...\n\n")
        box_file=client.folder(str(folderID)).upload(filePath,ntpath.basename(filePath))
        if verbose:
            print ("Uploaded file to box")
            print ("File Name: "+ box_file.name)
            print ("File ID: " + box_file.id)
            print ("In Folder:")
            print ("\tName: " + client.folder(str(folderID)).get()['name'])
            print ("\tID: " + str(folderID))
...
def refreshAuth():
    ...
    if verbose:
        ...
        print ("Status: "+ str(response.status_code))
        print ("Access Token:" + accessToken)
        print ("Expiration Time:" + str(expireTime))
        print ("\n\n")
